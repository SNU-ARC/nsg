...
run() {
  ...
  for k in ${K[@]}; do
    for l_size in ${L_SIZE[@]}; do
      ...
      echo "@@ Perform kNN searching using NSG index (L${l}K${k})" >> ${data}_baseline.log
      echo "@@ Perform kNN searching using NSG index (L${l}K${k})" >> ${data}_exact.log
      echo "@@ Perform kNN searching using NSG index (L${l}K${k})" >> ${data}_ours.log
      ...
      #echo "------------- exact --------------" >> ${data}_exact.log
      ./exact ${data}/${base}_base.fvecs ${data}/${base}_query.fvecs ${data}.nsg $l $k ${data}_nsg_result.ivecs ${data}/${base}_groundtruth.ivecs >> ${data}_exact.log
